...
/**
 * Landing page for the Bitbucket repo-connect OAuth popup.
 *
 * Flow:
 ...
 *
 * IMPORTANT: initAuth() is skipped when pathname === '/bitbucket/oauth/complete'
    ...
    useEffect(() => {
        ...
        try {
            localStorage.setItem(
                "__docnine_bitbucket_oauth_result",
        ...
        if (window.opener && !window.opener.closed) {
            try {
                window.opener.postMessage(
                    { type: "bitbucket-oauth-complete", status, user, msg },
        ...
        //    In that case redirect to /projects after a short wait.
        const fallback = setTimeout(() => {
            const qs = new URLSearchParams()
            if (status) qs.set("bitbucket", status)
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-background">
            <div className="flex flex-col items-center gap-4 text-center">
                ...
                <p className="text-sm text-muted-foreground">Completing Bitbucket connection…</p>
            </div>
        </div>
    )
}
