...
/**
 * Landing page for the Azure DevOps repo-connect OAuth popup.
 *
 * Flow:
 ...
 *
 * IMPORTANT: initAuth() is skipped when pathname === '/azure/oauth/complete'
        ...
        if (window.opener && !window.opener.closed) {
            try {
                window.opener.postMessage(
                    { type: "azure-oauth-complete", status, user, msg },
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-background">
            <div className="flex flex-col items-center gap-4 text-center">
                ...
                <p className="text-sm text-muted-foreground">Completing Azure DevOps connection…</p>
            </div>
        </div>
    )
}
